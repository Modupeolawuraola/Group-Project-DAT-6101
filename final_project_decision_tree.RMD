...
tree <- rpart(temp ~ .,method = 'poisson', data = copied_df,minsplit = 5)
...
rf.model <- randomForest(temp ~ ., method = 'poisson', data = copied_df)
print(rf.model)
...
<!-- indices <- sample(1:nrow(new_df), size = nrow(new_df), replace = FALSE) -->

<!-- train_percentage <- 0.8 -->

<!-- train_size <- round(nrow(new_df) * train_percentage) -->

<!-- train_set <- new_df[indices[1:train_size], ] -->

<!-- train_target <- target[indices[1:train_size]] -->

<!-- test_set <- new_df[indices[(train_size + 1):nrow(new_df)], ] -->

<!-- test_target <- target[indices[(train_size + 1):nrow(new_df)]] -->


<!-- library(class) -->

<!-- predicted_season <- knn(train_set,test_set,train_target, k = 1) -->


<!-- missclas.error <- mean(test_target != predicted_season) -->

<!-- print(missclas.error) -->


<!-- #choosing a k value -->

<!-- predicted_season <- NULL -->
<!-- error.rate <- NULL -->

<!-- for (i in 1:20){ -->
<!--   set.seed(101) -->
<!--   predicted_season <- knn(train_set, test_set, train_target, k = i) -->
  error.rate[i] <- mean(test_target != predicted_season)
